...
import api from '@/api/user-services-settings/user-broad-works-mobility-service'
import {
  UiCard,
  UiLoadingCard,
  UiButton,
...
export const UserBroadWorksMobility = ({ match }) => {
  ...
  const phonesRingTypes = options.phonesRingTypes || {}
  const userSettingLevelTypes = options.userSettingLevelTypes || {}
  ...
  async function update(formData) {
	  showLoadingModal()
    try {
		  const newBroadWorksMobility = await api.update(formData)
		   setQueryData(
		     ['broadworks-mobility'],newBroadWorksMobility,
			 {
				 shouldReFetch:true
			 }
		   )
